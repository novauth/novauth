...
passport.use(
  ...
  new passportJwt.Strategy(
    ...
    (jwtPayload, done) => {
      ...
      isUserBlacklisted(jwtPayload.email)
        .then((blockedUser) => {
          if (blockedUser) {
      ...
      isRefreshTokenValid(jwtPayload)
        .then(async (valid) => {
          if (valid) {
            ...
            return await invalidateRefreshTokens(jwtPayload.sub).then(() =>
              done(null, false)
            )
          }
        })
...
passport.use(
  'basic',
  new passportHttp.BasicStrategy((email, password, done) => {
    UserModel.findOne({ email }, { email: 1, password: 1 }, {}, (err, user) => {
      /* server error */
 ...
 * @param email the email which will be put in the subject claim
 * @returns the newly created access token as string
 */
function generateAccessToken(tokenData: object, email: string): string {
 ...
 */
async function generateRefreshToken(email: string): Promise<string> {
  ...
  await redis.setex(`USER_${email}_RT`, REFRESH_TOKEN_EXPIRY.SECONDS, jti)
  return jsonwebtoken.sign(
    ...
    {
      expiresIn: `${REFRESH_TOKEN_EXPIRY.SECONDS}s`,
      subject: email,
      jwtid: jti,
    }
  )
...
async function generateTokens(user: any): Promise<string[]> {
  ...
  const accessToken = generateAccessToken(tokenData, user.email)
  const refreshToken = await generateRefreshToken(user.email)
  return [accessToken, refreshToken]
}

/**
 * Check if the provided user has been recently blacklisted
 * @param email
 */
async function isUserBlacklisted(email: string): Promise<boolean> {
  const userBlacklisted = await redis.get(`USER_${email}_BL`)
 ...
 * @param email the user whose access and refresh tokens will be invalidated
 */
async function invalidateTokens(email: string, atJti: string): Promise<void> {
 ...
 * @param email the user whose refresh tokens will be invalidated
 */
async function invalidateRefreshTokens(email: string): Promise<boolean> {
  /*
  ...
  */
  await redis.del(`USER_${email}_RT`)
  return true
}
 ...
 * @param email email of the user to blacklist
 */
async function blacklistUser(email: string): Promise<void> {
  // add user to blacklist. this will allow to invalidate any currently valid access token
  await redis.setex(`USER_${email}_BL`, ACCESS_TOKEN_EXPIRY.SECONDS, '')
 ...
 * @param email email of the user to unblacklist
 */
async function unblacklistUser(email: string): Promise<void> {
  // remove the user from the blacklist, if the key is still present in cache
  await redis.del(`USER_${email}_BL`)
}
